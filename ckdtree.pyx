...
cdef class cKDTree:
    ...
    def qdens(cKDTree self, object x, object mass, np.intp_t k=1, np.float64_t phoc=-1., 
        ...
        Query the kd-tree for nearest neighbors and return SPH densities,
        we use the same kernel as Gadget-2.
        ...
        phoc : density cut for out put. Default, -1.0, which is not included.
            Otherwise, only particle with density > phoc is calculated. Others are 0.
            Note, this is cumulative density, not sph differential density
        eps : non-negative float
            Return approximate nearest neighbors; the kth returned value 
            is guaranteed to be no further than (1+eps) times the 
            distance to the real k-th nearest neighbor.
