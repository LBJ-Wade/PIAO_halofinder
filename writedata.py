...
def writedata(wpath,rpath,exts,spnm,binsize,ii,lgid,edn=None):
    ...
    for i in range(npart.size):
        for j in range(cumpart[i],cumpart[i+1]):
            mpart[xyz[j],i]+=1
        xyz[cumpart[i]:cumpart[i+1]]=np.argsort(xyz[cumpart[i]:cumpart[i+1]])+cumpart[i]
        #H[xyz[i]]+=1

    H=np.sum(mpart,axis=1,dtype=np.uint32)
    hcum=np.cumsum(mpart,axis=0)
    #pot=readsnapsgl(rpath+"/snapdir_"+exts+"/"+spnm+exts+"."+str(ii),"POT ",endian=edn,quiet=1,fmt=0)
    ...
    for i in np.arange(bins3):
        ...
        if H[i]>0:
            ff.write(mpart[i,:])
            tmpos=np.array([],dtype='int32')
            for j in range(npart.size):
                if hcum[i,j]>0:
                    tmpos=np.append(tmpos,xyz[hcum[i,j]-mpart[i,j]+cumpart[j]:hcum[i,j]+cumpart[j]])
            ff.write(ID[tmpos])
            ff.write(pos[tmpos,:])
            if len(mass) !=1:
                ff.write(mass[tmpos])
...
def readdata_smp(ii,tmpph,boxsize,binsize,bufsize,fnum,lgid):
    ...
    return meshids,meshpos,meshpot,meshmas,meshnum
